jQuery(document).ready(function() {
    ...
    if (button == 'button') { 
        ...
        var btn_result = '<a id="tpbr_calltoaction" style="background:' + ltbcolor + '; display:inline-block; padding:0px 10px 2px; color:white; text-decoration:none; margin: 0px 20px 1px;border-radius:3px; line-height:29px;" href="' + url + '">' + link + '</a>';   
    } else {
        var btn_result = '';
    }  
    ...
    if (status == 'active') {
        if (fixed == 'fixed'){
            jQuery('<div style="height:44px; visibility:hidden;"></div><div id="tpbr_topbar" style="' + fixed_result + ' background:' + tbcolor + '; padding:3px 20px 5px;"><div id="tpbr_box" style="line-height:40px; font-size:15px; font-family: Helvetica, Arial, sans-serif; text-align:center; width:100%; color:white; font-weight:300;">' + message + btn_result + '</div></div>').prependTo('body');
        } else {
            jQuery('<div id="tpbr_topbar" style="' + fixed_result + ' background:' + tbcolor + '; padding:3px 20px 5px;"><div id="tpbr_box" style="line-height:40px; text-align:center; width:100%; color:white; font-size:15px; font-family: Helvetica, Arial, sans-serif;  font-weight:300;">' + message + btn_result + '</div></div>').prependTo('body');
        }    
    }
    
    jQuery('.tpbr_close').click(function() {
        jQuery('#tpbr_topbar').slideUp(100);
    });
